...
class WeatherPlugin(BasePlugin):
    ...
    def initialize(self) -> None:
        ...
        print(f"Weather plugin initialized with units: {self.config.config.get('units', 'metric')}")
    ...
    async def _call_weather_api(self, endpoint: str, params: Dict[str, Any]) -> Dict[str, Any]:
...
class WeatherAgent:
    ...
    async def analyze_weather_trends(self, locations: List[str]) -> Dict[str, Any]:
        ...
        for location in locations:
            ...
            weather_tool = next((t for t in self.tools if t.name == "get_weather"), None)
            if weather_tool:
