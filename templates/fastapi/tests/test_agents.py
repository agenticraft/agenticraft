...
class TestAgentEndpoints:
    ...
    async def test_simple_agent(self, mock_run, auth_headers):
        ...
        response = client.post(
            ...
            json={"prompt": "Hello, how are you?", "context": {"user_id": "test-user"}},
        )
    ...
    async def test_reasoning_agent(self, mock_think_and_act, auth_headers):
        ...
        response = client.post(
            "/agents/reasoning",
            headers=auth_headers,
            json={"prompt": "Plan a sustainable city", "options": {"max_steps": 5}},
        )
    ...
    def test_missing_auth(self):
        ...
        response = client.post("/agents/simple", json={"prompt": "Hello"})
    ...
    def test_invalid_prompt(self, prompt, auth_headers):
        ...
        response = client.post(
            "/agents/simple", headers=auth_headers, json={"prompt": prompt}
        )
    ...
    def test_rate_limiting(self, auth_headers):
        ...
        for i in range(5):
            response = client.post(
                "/agents/simple", headers=auth_headers, json={"prompt": f"Test {i}"}
            )
