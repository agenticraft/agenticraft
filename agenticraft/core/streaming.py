...
class StreamInterruptedError(AgentError):
    ...
    def __init__(self, message: str = "Stream was interrupted", partial_response: Optional[str] = None):
...
class StreamingManager:
    ...
    async def stream_with_timeout(
        self,
        stream_coro: AsyncIterator[StreamChunk],
        ...
        if timeout:
            try:
                ...
                raise StreamInterruptedError(
                    f"Stream timeout after {timeout} seconds"
                )
        else:
            async for chunk in stream_coro:
                yield chunk
...
def create_mock_stream(text: str, chunk_size: int = 10, delay: float = 0.1) -> AsyncIterator[StreamChunk]:
