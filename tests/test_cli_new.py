...
from agenticraft.cli.commands.new import (
    new, _get_template_path, _copy_template, _init_git, _update_project_files,
    AVAILABLE_TEMPLATES
)
...
class TestNewCommand:
    ...
    def test_new_with_template(self, runner, temp_dir):
        ...
        with runner.isolated_filesystem(temp_dir=temp_dir):
            ...
            with patch('agenticraft.cli.commands.new._get_template_path', return_value=mock_template_path):
                result = runner.invoke(new, ['my-api', '--template', 'fastapi'])
                
                assert result.exit_code == 0
                assert "Creating new fastapi project: my-api" in result.output
                assert "docker-compose up" in result.output  # FastAPI specific instruction
    ...
    def test_new_copy_error_cleanup(self, runner, temp_dir):
        ...
        with runner.isolated_filesystem(temp_dir=temp_dir):
            ...
            with patch('agenticraft.cli.commands.new._get_template_path', return_value=mock_template_path):
                with patch('agenticraft.cli.commands.new._copy_template', side_effect=Exception("Copy failed")):
                    result = runner.invoke(new, ['my-project'])
...
class TestNewCommandHelpers:
    ...
    def test_get_template_path_dev_mode(self, tmp_path):
        ...
        dev_path = tmp_path / "templates" / "basic"
        dev_path.mkdir(parents=True)
        
        with patch('pathlib.Path.exists', return_value=True):
            with patch('agenticraft.cli.commands.new.Path') as mock_path:
                # Set up the mock to return our test path
                mock_file = MagicMock()
                mock_file.parent.parent.parent.parent = tmp_path
                mock_path.return_value = mock_file
                mock_path.__file__ = mock_file
                
                # Also need to handle the actual path construction
    ...
    def test_update_project_files(self, tmp_path):
        ...
        pyproject.write_text('name = "agenticraft-template"\n[project]\nname = "agenticraft-template"')
