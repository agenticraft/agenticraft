...
class TestOllamaProvider:
    ...
    def test_initialization_with_params(self):
        ...
        provider = OllamaProvider(
            base_url="http://192.168.1.100:11434", model="codellama", timeout=600
        )
    ...
    async def test_complete_with_parameters(self, provider, mock_response):
        ...
        await provider.complete(
            messages, temperature=0.2, max_tokens=100, top_p=0.9, seed=42
        )
    ...
    async def test_pull_model(self, provider, mock_response):
        ...
        mock_client.post.assert_called_once_with(
            "/api/pull", json={"name": "llama2"}, timeout=None
        )
    ...
    async def test_complete_without_usage_data(self, provider, mock_response):
        ...
        mock_response.json.return_value = {
            "message": {"role": "assistant", "content": "Response without token data"}
