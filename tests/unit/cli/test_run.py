...
"""
    ...
    def test_run_python_agent_no_prompt(self, runner, temp_agent_py):
        ...
        with patch("importlib.util.spec_from_file_location", return_value=mock_spec):
            with patch("importlib.util.module_from_spec", return_value=mock_module):
                ...
                assert result.exit_code != 0
                assert (
                    "Error: --prompt required for non-interactive mode" in result.output
                )
    ...
    def test_run_python_agent_interactive(self, runner, temp_agent_py):
        ...
        with patch("importlib.util.spec_from_file_location", return_value=mock_spec):
            with patch("importlib.util.module_from_spec") as mock_module:
                ...
                with patch(
                    "agenticraft.cli.commands.run._run_interactive"
                ) as mock_interactive:
        ...
        with patch("agenticraft.Agent", return_value=mock_agent):
            with patch("agenticraft.providers.get_provider") as mock_get_provider:
                result = runner.invoke(
                    run, [str(temp_agent_yaml), "--provider", "anthropic"]
                )
    ...
    def test_run_yaml_reasoning_agent(self, runner, tmp_path):
        ...
        config = {
            "agent": {"name": "ReasoningBot", "type": "reasoning", "provider": "openai"}
        }
        config_file.write_text(yaml.dump(config))

        mock_agent = Mock()
        mock_result = Mock(
            content="Reasoned response", reasoning=Mock(synthesis="My reasoning")
        )
        mock_agent.run = AsyncMock(return_value=mock_result)
...
class TestRunCommandHelpers:
            ...
            with pytest.raises(click.Abort):
                with patch("click.echo") as mock_echo:
                    ...
                    mock_echo.assert_called_with(
                        f"Error: Failed to load {file_path}", err=True
                    )
            ...
            with patch("importlib.util.module_from_spec"):
                with pytest.raises(click.Abort):
                    with patch("click.echo") as mock_echo:
                        _run_python_agent(file_path, None, False, None, None)
                        mock_echo.assert_called_with(
                            "Error loading agent: Syntax error", err=True
                        )
    ...
    def test_run_yaml_agent_with_tools(self):
        ...
        config = {"agent": {"name": "ToolAgent", "tools": ["calculator", "search"]}}
    ...
    def test_run_interactive(self):
        ...
        with patch("click.prompt", side_effect=["Hello bot", "quit"]):
            with patch("click.echo") as mock_echo:
                with patch(
                ...
                ):  # Mock confirm to avoid stdin issues
                    ...
                    assert any(
                        "TestBot: Interactive response" in arg for arg in echo_args
                    )
    ...
    def test_run_interactive_error_handling(self):
        ...
        with patch("click.prompt", side_effect=["Cause error", "quit"]):
            with patch("click.echo") as mock_echo:
                ...
                echo_err_args = [
                    str(call.args[0]) if call.args and call.kwargs.get("err") else ""
                    for call in mock_echo.call_args_list
                ]
                assert any(
                    "Error: Agent error" in arg for arg in echo_args + echo_err_args
                )
