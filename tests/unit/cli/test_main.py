...
class TestCLIMain:
    ...
    def test_info_command_provider_error(self, runner):
        ...
        with patch(
            "agenticraft.providers.list_providers", side_effect=Exception("Load error")
        ):
            result = runner.invoke(cli, ["info"])
            assert result.exit_code == 0
    ...
    def test_context_object(self, runner):
        ...
        # The CLI should initialize ctx.obj as a dict
        # We can verify this by running any command successfully
        # If ctx.obj wasn't properly initialized, commands would fail

        # Test version command which is simple and doesn't require external dependencies
        result = runner.invoke(cli, ["version"])
        assert result.exit_code == 0
        assert f"AgentiCraft {__version__}" in result.output

        # Test info command as well
        result = runner.invoke(cli, ["info"])
        assert result.exit_code == 0

        # The fact that these commands run successfully means ctx.ensure_object(dict)
        # in the main CLI is working properly
