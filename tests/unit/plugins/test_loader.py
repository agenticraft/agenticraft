...
from agenticraft.plugins.base import BasePlugin, PluginConfig, PluginInfo
...
class MockPluginForLoading(BasePlugin):
    ...
    def get_info(self) -> PluginInfo:
        return PluginInfo(
            name=self.name, version=self.version, description="Test plugin for loading"
        )
...
class TestPluginLoader:
    ...
    def test_loader_creation(self):
        ...
        # When auto_discover=False, it should not add default dirs
        # But it might still have user plugin dir
        assert len(loader.plugin_dirs) <= 1  # At most the user dir
        if loader.plugin_dirs:
            # Should be the user plugins directory
            assert ".agenticraft/plugins" in str(loader.plugin_dirs[0])
...
"""
    ...
    def test_load_all_discovered(self):
        ...
        loader._plugin_classes["plugin2"] = type(
            "Plugin2", (MockPluginForLoading,), {"name": "plugin2"}
        )
    ...
    def test_list_plugins(self):
        ...
        loader._plugin_classes["plugin1"] = MockPluginForLoading
        loader._plugin_classes["plugin2"] = type(
            "Plugin2", (MockPluginForLoading,), {"name": "plugin2", "version": "2.0.0"}
        )
...
class TestGlobalFunctions:
    ...
    def test_load_plugin_global(self):
        ...
        loader._plugin_classes["test_global"] = type(
            "GlobalPlugin", (MockPluginForLoading,), {"name": "test_global"}
        )
