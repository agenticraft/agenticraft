...
class TestWorkflowStep:
    ...
    def test_can_run_with_dependencies(self):
        ...
        step = WorkflowStep(name="dependent", depends_on=["step1", "step2"])
...
class TestWorkflow:
    ...
    def test_add_step(self):
        ...
        step1 = workflow.add_step(name="step1", action="First action")

        step2 = workflow.add_step(
            name="step2", action="Second action", depends_on=["step1"]
        )
    ...
    def test_get_ready_steps(self):
        ...
        # Add dependent step
        step3 = workflow.add_step(name="step3", depends_on=["step1", "step2"])
...
class TestWorkflowAgent:
    ...
    def agent(self):
        ...
        return WorkflowAgent(
            name="TestWorkflowAgent", instructions="Test workflow agent"
        )
    ...
    def test_create_workflow(self, agent):
        ...
        workflow = agent.create_workflow(
            name="test_workflow", description="Test description"
        )
    ...
    async def test_execute_workflow_simple(self, agent):
        ...
        with patch.object(agent, "arun", new_callable=AsyncMock) as mock_arun:
            ...
            assert all(
                r.status == StepStatus.COMPLETED for r in result.step_results.values()
            )
    ...
    async def test_execute_parallel_workflow(self, agent):
        ...
        workflow.add_step(
            name="final", action="Final task", depends_on=["parallel1", "parallel2"]
        )
        ...
        async def mock_arun(prompt):
            step_name = (
                "parallel1"
                if "Task 1" in prompt
                else ("parallel2" if "Task 2" in prompt else "final")
            )
    ...
    async def test_step_with_custom_handler(self, agent):
        ...
        # Execute
        result = await agent.execute_workflow(workflow, context={"input": "test"})
    ...
    async def test_step_with_condition(self, agent):
        ...
        with patch.object(agent, "arun", new_callable=AsyncMock) as mock_arun:
            ...
            result = await agent.execute_workflow(workflow, context={"mode": "run"})

            assert result.step_results["always_run"].status == StepStatus.COMPLETED
            assert (
                result.step_results["conditional"].status == StepStatus.SKIPPED
            )  # Should be skipped when condition is false
            ...
            result = await agent.execute_workflow(workflow, context={"mode": "skip"})

            assert result.step_results["always_run"].status == StepStatus.COMPLETED
            assert (
                result.step_results["conditional"].status == StepStatus.COMPLETED
            )  # Should be completed when condition is true
    ...
    async def test_step_timeout(self, agent):
        ...
        workflow.add_step(
            name="slow_step", action="Slow task", timeout=0.1  # 100ms timeout
        )
    ...
    async def test_step_retry(self, agent):
        ...
        workflow.add_step(name="flaky_step", action="Flaky task", max_retries=2)
...
class TestWorkflowResult:
    ...
    def test_result_creation(self):
        ...
        step_results = {
            "step1": StepResult(
                name="step1", status=StepStatus.COMPLETED, result="Result 1"
            ),
            "step2": StepResult(
                name="step2", status=StepStatus.FAILED, error="Error occurred"
            ),
        }
    ...
    def test_format_summary(self):
        ...
        step_results = {
            "fetch": StepResult(
                name="fetch", status=StepStatus.COMPLETED, duration=1.2
            ),
            "process": StepResult(
                name="process",
                status=StepStatus.FAILED,
            ...
            "save": StepResult(name="save", status=StepStatus.SKIPPED),
        }
        ...
        assert (
            "❌ process: StepStatus.FAILED (0.50s) - Error: Processing error" in summary
        )
