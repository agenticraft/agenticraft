...
class TestSimpleCalculateTool:
    ...
    def test_math_functions(self):
        ...
        assert simple_calculate.run(expression="sqrt(2)") == pytest.approx(
            1.4142, rel=0.001
        )
        ...
        assert simple_calculate.run(expression="exp(1)") == pytest.approx(
            math.e, rel=0.001
        )
    ...
    def test_complex_expressions(self):
        ...
        assert simple_calculate.run(expression=f"pi * {radius}**2") == pytest.approx(
            expected_area
        )
    ...
    def test_invalid_expressions(self):
        ...
        with pytest.raises(
            (ToolExecutionError, ValueError), match="Failed to evaluate"
        ):
    ...
    def test_non_numeric_result(self):
        ...
        with pytest.raises(
            (ToolExecutionError, ValueError), match="Result must be numeric"
        ):
