...
def create_mock_step(name: str, agent=None, tool=None, inputs=None, depends_on=None, retry_count=0, timeout=None):
...
class TestWorkflow:
    ...
    def test_add_multiple_steps(self):
        ...
        steps = [
            Step.model_construct(name="step1", agent=agent1, inputs={"task": "Task 1"}, depends_on=[], retry_count=0, timeout=None),
            Step.model_construct(name="step2", agent=agent2, inputs={"task": "Task 2"}, depends_on=[], retry_count=0, timeout=None)
        ]
    ...
    def test_duplicate_step_names(self):
        ...
        step1 = Step.model_construct(name="duplicate", agent=agent, inputs={}, depends_on=[], retry_count=0, timeout=None)
        step2 = Step.model_construct(name="duplicate", agent=agent, inputs={}, depends_on=[], retry_count=0, timeout=None)
    ...
    def test_get_execution_order_simple(self):
        ...
        steps = [
            Step.model_construct(name="step1", agent=agent, inputs={}, depends_on=[], retry_count=0, timeout=None),
            Step.model_construct(name="step2", agent=agent, inputs={}, depends_on=["step1"], retry_count=0, timeout=None),
            Step.model_construct(name="step3", agent=agent, inputs={}, depends_on=["step2"], retry_count=0, timeout=None)
        ]
    ...
    def test_get_execution_order_parallel(self):
        ...
        steps = [
            Step.model_construct(name="parallel1", agent=agent, inputs={}, depends_on=[], retry_count=0, timeout=None),
            Step.model_construct(name="parallel2", agent=agent, inputs={}, depends_on=[], retry_count=0, timeout=None),
            Step.model_construct(name="parallel3", agent=agent, inputs={}, depends_on=[], retry_count=0, timeout=None),
            Step.model_construct(name="final", agent=agent, inputs={}, 
                 depends_on=["parallel1", "parallel2", "parallel3"], retry_count=0, timeout=None)
        ]
    ...
    def test_circular_dependency_detection(self):
        ...
        steps = [
            Step.model_construct(name="step1", agent=agent, inputs={}, depends_on=["step3"], retry_count=0, timeout=None),
            Step.model_construct(name="step2", agent=agent, inputs={}, depends_on=["step1"], retry_count=0, timeout=None),
            Step.model_construct(name="step3", agent=agent, inputs={}, depends_on=["step2"], retry_count=0, timeout=None)
        ]
    ...
    async def test_workflow_execution_with_dependencies(self):
        ...
        steps = [
            Step.model_construct(name="step1", agent=agent1, inputs={"task": "First task"}, depends_on=[], retry_count=0, timeout=None),
            Step.model_construct(name="step2", agent=agent2, 
                 inputs={"task": "Use $step1", "data": "$step1"},
                 depends_on=["step1"], retry_count=0, timeout=None)
        ]
    ...
    def test_workflow_visualize(self):
        ...
        steps = [
            Step.model_construct(name="start", agent=agent, inputs={}, depends_on=[], retry_count=0, timeout=None),
            Step.model_construct(name="process1", agent=agent, inputs={}, depends_on=["start"], retry_count=0, timeout=None),
            Step.model_construct(name="process2", agent=agent, inputs={}, depends_on=["start"], retry_count=0, timeout=None),
            Step.model_construct(name="end", agent=agent, inputs={}, 
                 depends_on=["process1", "process2"], retry_count=0, timeout=None)
        ]
