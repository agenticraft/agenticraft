...
class TestToolDecorator:
    ...
    def test_tool_decorator_with_complex_types(self):
        ...
        @tool
        def process_list(
            items: list[str], options: dict[str, bool] | None = None
        ) -> dict[str, list[str]]:
            """Process a list of items."""
            options = options or {}
            return {"original": items, "processed": [item.upper() for item in items]}
...
class TestFunctionTool:
    ...
    def test_tool_creation(self):
        ...
        tool_instance = FunctionTool(
            func=sample_func, name="doubler", description="Doubles the input"
        )
