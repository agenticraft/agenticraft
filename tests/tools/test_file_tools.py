...
class TestWriteFileTool:
    ...
    async def test_write_new_file(self, temp_dir):
        ...
        result = await write_file.arun(path=str(file_path), content="New content", overwrite=True)
    ...
    async def test_overwrite_protection(self, sample_files):
        ...
        with pytest.raises(ToolExecutionError, match="File already exists"):
            await write_file.arun(path=str(sample_files['text_file']), content="New content")
    ...
    async def test_create_parent_directories(self, temp_dir):
        ...
        result = await write_file.arun(path=str(nested_path), content="Nested content", overwrite=True)
    ...
    async def test_write_with_encoding(self, temp_dir):
        ...
        result = await write_file.arun(path=str(file_path), content=content, encoding="utf-8", overwrite=True)
...
class TestListFilesTool:
    ...
    async def test_list_with_pattern(self, sample_files):
        ...
        # List only .txt files
        txt_files = await list_files.arun(directory=str(sample_files['temp_dir']), pattern="*.txt")
        assert len(txt_files) == 1
        assert txt_files[0]['name'] == 'sample.txt'
        
        # List only .json files
        json_files = await list_files.arun(directory=str(sample_files['temp_dir']), pattern="*.json")
    ...
    async def test_list_recursive(self, sample_files):
        ...
        files = await list_files.arun(directory=str(sample_files['temp_dir']), recursive=True)
    ...
    async def test_list_include_hidden(self, sample_files):
        ...
        files = await list_files.arun(
            directory=str(sample_files['temp_dir']), 
            include_hidden=True
        )
...
class TestJsonTools:
    ...
    async def test_write_json_formatting(self, temp_dir):
        ...
        await write_json.arun(path=str(file_path), data=data, indent=None, overwrite=True)
