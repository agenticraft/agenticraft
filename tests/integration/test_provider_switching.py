...
from agenticraft.core.exceptions import AgentError, ProviderError
...
class TestProviderSwitching:
    ...
    def mock_openai_response(self):
        ...
        return MagicMock(
            choices=[
                MagicMock(
                    message=MagicMock(content="OpenAI response", tool_calls=None),
                    finish_reason="stop",
                )
            ],
    ...
    async def test_switch_providers_on_agent(self, mock_env):
        ...
        agent = Agent(name="SwitchableAgent", model="gpt-4", provider="openai")
    ...
    async def test_provider_specific_features(self, mock_env):
        ...
        with patch.object(agent.provider, "complete") as mock_complete:
            mock_complete.return_value = CompletionResponse(
                content='{"answer": "42"}', tool_calls=[], finish_reason="stop"
            )

            await agent.arun("Give me JSON", response_format={"type": "json_object"})
        ...
        with patch.object(agent.provider, "complete") as mock_complete:
            mock_complete.return_value = CompletionResponse(
                content="I am Claude", tool_calls=[], finish_reason="stop"
            )
        ...
        with patch.object(agent.provider, "complete") as mock_complete:
            mock_complete.return_value = CompletionResponse(
                content="Local response", tool_calls=[], finish_reason="stop"
            )
    ...
    @pytest.mark.skip("Implementation changed - no longer raises exception")
    ...
    async def test_concurrent_agents_different_providers(self, mock_env):
        ...
        # Mock responses
        with (
            patch.object(OpenAIProvider, "complete") as mock_openai,
    ...
    def test_provider_factory_integration(self, mock_env):
        ...
        openai = ProviderFactory.create(model="gpt-4", api_key="sk-test-key-123456")
        assert isinstance(openai, OpenAIProvider)

        anthropic = ProviderFactory.create(
            model="claude-3-opus-20240229", api_key="sk-ant-test-key-123456"
        )
        assert isinstance(anthropic, AnthropicProvider)

        ollama = ProviderFactory.create(model="llama2")  # or "ollama/llama2"
...
class TestProviderFeatureCompatibility:
    ...
    async def test_streaming_across_providers(self, mock_all_providers):
        ...
        for provider in ["openai", "anthropic"]:
            ...
            with patch.object(agent.provider, "complete") as mock_complete:
                mock_complete.return_value = CompletionResponse(
                    content="Streamed response", tool_calls=[], finish_reason="stop"
                )
    ...
    async def test_tool_calling_across_providers(self, mock_all_providers):
        ...
        with patch.object(agent.provider, "complete") as mock_complete:
            ...
            tool_response = CompletionResponse(
                ...
                tool_calls=[
                    ToolCall(
                        id="call_456", name="get_weather", arguments={"city": "London"}
                    )
                ],
        ...
        with patch.object(agent.provider, "complete") as mock_complete:
            ...
            async def slow_complete(*args, **kwargs):
                ...
                return CompletionResponse(
                    content="Completed", tool_calls=[], finish_reason="stop"
                )
