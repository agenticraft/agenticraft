...
class TestMCPIntegration:
    ...
    async def test_basic_mcp_client(self):
        ...
        # Import the example
        import importlib.util
        spec = importlib.util.spec_from_file_location("basic_client", "examples/mcp/basic_client.py")
        ...
        await module.call_specific_tool("ws://localhost:3000", "test_tool", test_param="value")
    ...
    async def test_basic_mcp_server(self):
        ...
        # Import the example
        import importlib.util
        spec = importlib.util.spec_from_file_location("basic_server", "examples/mcp/basic_server.py")
        module = importlib.util.module_from_spec(spec)
        spec.loader.exec_module(module)
        ...
        # IMPORTANT: Do NOT run the actual server as it will block forever
        # Instead, we'll mock the server startup to prevent hanging
        with patch.object(module, 'run_websocket_server', new_callable=AsyncMock) as mock_ws_server:
            with patch.object(module, 'run_http_server', new_callable=AsyncMock) as mock_http_server:
                # Mock the server startup to return immediately
                mock_ws_server.return_value = None
                mock_http_server.return_value = None
                
                # Test with websocket mode
                with patch('sys.argv', ['basic_server.py', 'websocket']):
                    # This should now return immediately instead of blocking
                    await module.main()
                    # Verify websocket server was called
                    mock_ws_server.assert_called_once()
                
                # Reset mocks
                mock_ws_server.reset_mock()
                mock_http_server.reset_mock()
                
                # Test with http mode
                with patch('sys.argv', ['basic_server.py', 'http']):
                    # Mock uvicorn to be available
                    with patch.dict('sys.modules', {'uvicorn': MagicMock()}):
                        await module.main()
                        # Verify HTTP server was called
...
class TestMCPToolIntegration:
    ...
    async def test_mcp_agent_integration(self):
        ...
        from agenticraft.protocols.mcp.client import MCPClient
