...
class TestMCPServer:
    ...
    def server(self):
        """Create test server."""
        server = MCPServer(
            name="Test Server", version="1.0.0", description="Test MCP Server"
        )
    ...
    async def test_handle_initialize(self, server):
        ...
        request = MCPRequest(
            method=MCPMethod.INITIALIZE, params={"client": "test", "version": "1.0"}
        )
    ...
    async def test_handle_describe_tool(self, server):
        ...
        request = MCPRequest(
            method=MCPMethod.DESCRIBE_TOOL, params={"name": "test_calculate"}
        )
    ...
    async def test_handle_call_tool(self, server):
        ...
        request = MCPRequest(
            ...
            params={"tool": "test_calculate", "arguments": {"expression": "2 + 3"}},
        )
    ...
    async def test_handle_invalid_method(self, server):
        ...
        request = MCPRequest(method="invalid/method", id="test-123")  # type: ignore
    ...
    async def test_handle_tool_not_found(self, server):
        ...
        request = MCPRequest(
            method=MCPMethod.CALL_TOOL, params={"tool": "nonexistent", "arguments": {}}
        )
            ...
            json.dumps(
                {
                    "jsonrpc": "2.0",
                    "method": "tools/call",
                    "params": {"tool": "test_echo", "arguments": {"message": "hello"}},
                    "id": 2,
                }
            ),
        ]
